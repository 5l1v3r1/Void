#pragma once
#ifndef _V_REDBLACKTREE_H_
#define _V_REDBLACKTREE_H_
...
namespace Void
{
    ...
    protected:
        ...
        public:
            ...
            //----------------------------------------------------------------------------------------------------
            inline virtual void Calculate() // Dirction: ↑
            {
                // Nothing
            }
    ...
    protected:
        ...
        inline Node* FindNode(Node* _node, Node* _refer, bool isKeyOnly=false)
        {
            if (_node && _refer)
            {
                ...
                {
                    if (isKeyOnly || _node->IsEqual(_refer))
                        return _node;
                    else
                    {
                        Node *result = nullptr;
                        result = FindNode(_node->left, _refer, isKeyOnly);
                        if (!result)
                            result = FindNode(_node->right, _refer, isKeyOnly);
                        return result;
                    }
                }
                else if (_refer->IsKeyLess(_node))
                    return FindNode(_node->left, _refer, isKeyOnly);
                else
                    return FindNode(_node->right, _refer, isKeyOnly);
            }
